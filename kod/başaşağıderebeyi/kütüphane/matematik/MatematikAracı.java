...
/** Ondalıklı ve tam sayıların aritmetiğiyle ilgili yardımcı yöntemler sağlar. İkinci işlevi Java'nın
 * Math sınıfındaki methodlardan float alıp da double döndüren yöntemlerin sonuçlarını floata
 * çevirmektir. Buradaki yöntemlerin hespi sınıfa aittir. Bu sınıfın nesnesi oluşturulmaz. */
public class MatematikAracı {
	private static final float RADYANA_ÇEVİREN_ÇARPAN = (float)(Math.PI / 180.0D);
	private static final float DERECEYE_ÇEVİREN_ÇARPAN = 1.0F / RADYANA_ÇEVİREN_ÇARPAN;
	private static final float DENK_SAYILACAK_ÖZÜR_PAYI = 0.1F;
	
	/** Benek biriminden verilen iki sayının denk sayılıp sayılmadığını döndürür. Denklik, tam olarak
	 * eşitlik değilse de aşırı yakınlıktır. */
	public static boolean denkSayılmalarınıBul(final float soldakiSayı, final float sağdakiSayı) {
		return Math.abs(soldakiSayı - sağdakiSayı) <= DENK_SAYILACAK_ÖZÜR_PAYI;
	}
	
	/** Verilen ondalık sayıyı aralıkta olacak şekilde döndürür. Sayı eğer aralıktaysa onu, taban değerin
	 * altındaysa taban değeri veya tavan değerin üstündeyse tavan değeri döndürür. */
	public static float sıkıştır(final float sayı, final float tabanDeğeri, final float tavanDeğeri) {
		return küçüğüBul(büyüğüBul(sayı, tabanDeğeri), tavanDeğeri);
	}
	
	/** Verilen iki ondalık sayıdan küçük olanı döndürür. */
	public static float küçüğüBul(final float soldakiSayı, final float sağdakiSayı) {
		return soldakiSayı < sağdakiSayı ? soldakiSayı : sağdakiSayı;
	}
	
	/** Verilen iki ondalık sayıdan büyük olanı döndürür. */
	public static float büyüğüBul(final float soldakiSayı, final float sağdakiSayı) {
		return soldakiSayı > sağdakiSayı ? soldakiSayı : sağdakiSayı;
	}
	...
	public static <T> T aradeğerleriniBul(	final T soldakiNesne,
	...
	/** Verilen iki tam sayının verilen mesafelere göre aradeğerini döndürür. Mesafelerin toplamının bir
	 * olduğunu varsayar. */
	public static int aradeğerleriniBul(final int soldakiTamSayı,
										final int sağdakiTamSayı,
										final float soldakineOlanUzaklık,
										final float sağdakineOlanUzaklık) {
		return yuvarla(aradeğerleriniBul(	(float)soldakiTamSayı,
											(float)sağdakiTamSayı,
											soldakineOlanUzaklık,
											sağdakineOlanUzaklık));
	}
	
	/** Verilen iki ondalık sayının verilen mesafelere göre aradeğerini döndürür. Mesafelerin toplamının
	 * bir olduğunu varsayar. */
	public static float aradeğerleriniBul(	final float soldakiSayı,
											final float sağdakiSayı,
											final float soldakineOlanUzaklık,
											final float sağdakineOlanUzaklık) {
		return soldakiSayı * sağdakineOlanUzaklık + sağdakiSayı * soldakineOlanUzaklık;
	}
