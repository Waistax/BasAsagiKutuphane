...
public class KodDeneme {
	...
	private static void sınıfTürüKontrolüDenemesi() {
		/*
		 * HATA:
		 * Cannot perform instanceof check against parameterized type Class<? extends Olay>.
		 * Use the form Class<?> instead since further generic type information will be erased at runtime
		 */
		//System.out.println(KodDeneme.class instanceof Class<? extends Olay>);
	}
	
	/** Yineleyici nesnesinin nasıl ilerlediğini dener. */
	@SuppressWarnings("unused")
	private static void yineleyiciİlerlemeDenemesi() {
		Set<String> küme = new HashSet<>();
		küme.add("Cem");
		küme.add("GEÇGEL");
		küme.add("mükemmel");
		küme.add("birisidir.");
		Iterator<String> yineleyici = küme.iterator();
	...
	private static void olayDenemesi() {
		OlayDağıtıcısı dağıtıcı = new OlayDağıtıcısı();
		DenemeDinleyicisi dinleyici = new DenemeDinleyicisi();
